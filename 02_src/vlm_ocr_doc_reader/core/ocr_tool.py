...
class OCRTool:
    ...
    Used by VLMAgent through function calling mechanism.
    Fetches page image from StateManager by page number so the agent only
    passes page_num and prompt.
    """
    ...
    def to_tool_definition(self) -> Dict:
        ...
        return {
            ...
            "function": {
                "name": "ask_ocr",
                "description": (
                    "Точное извлечение ОДНОГО конкретного значения со страницы "
                    "документа через OCR. Используй ТОЛЬКО когда нужна посимвольная "
                    "точность: URL, email, номера (ОГРН, ИНН, КПП, телефон), ФИО, "
                    "адреса, коды документов. НЕ используй для общего извлечения "
                    "текста — это делай сам. Один вызов = одно конкретное значение "
                    "с одной страницы."
                ),
                "parameters": {
                    "type": "object",
